...
proc mptigatext { nick host hand chan text } {
	global path linkdl durasi judul mptigaruncmd jdmp3
	...
	limitdownloadmp3txt $nick $host $hand $chan $text
	foreach line [split $mptigaruncmd "\n"] {
		if {[string match "ERROR:*" $line]} {
			if {[string match "*unable to download video data*" $line]} {
				limitdownloadmp3txt $nick $host $hand $chan $text
			} else {
...
proc mptigalink { nick host hand chan text } {
	global path linkdl durasi id judul mptigaruncmdd
	...
	putquick "PRIVMSG $chan :Trying to convert \002$judul\002 please wait..."
	limitdownloadmp3link $nick $host $hand $chan $text
...
proc mpempattext { nick host hand chan text } {
	global path linkdl durasi judul mpempatruncmd
	if {[string match "* -*" [string tolower $text]]} { regsub {\-} $text {} text }
	search $nick $host $hand $chan $text
	autoinfo $nick $host $hand $chan $text
	if {$durasi == "stream"} { puthelp "PRIVMSG $chan :Streaming is not allowed." ; return 0 }
	if {[string match "*:*" $durasi]} { set durasi "$durasi minutes" }
	putquick "PRIVMSG $chan :Trying to convert \002$judul\002 please wait..."
	limitdownloadmp4txt $nick $host $hand $chan $text
...
proc mpempatlink { nick host hand chan text } {
	global path linkdl durasi id judul mpempatruncmdd
	if {[regexp -nocase -- {(?:http(?:s|).{3}|)(?:www.|)(?:youtube.com\/watch\?.*v=|youtu.be\/)([\w-]{11})} $text url id]} {
		autoinfo $nick $host $hand $chan $text
		if {$durasi == "stream"} { puthelp "PRIVMSG $chan :Streaming is not allowed." ; return 0 }
		if {[string match "*:*" $durasi]} { set durasi "$durasi minutes" }
	...
	putquick "PRIVMSG $chan :Trying to convert \002$judul\002 please wait..."
	limitdownloadmp4link $nick $host $hand $chan $text
...
proc pub_update {nick host hand chan arg} {
	...
	foreach x [split $updated) "\n"] {
		if {[string match "*ERROR: *" $x]} {
			puthelp "PRIVMSG $chan :$x" ; return 0
...
proc ganti_size {nick host hand chan text} {
	...
	if {[lindex $text 0] == "mp3"} {
		...
		puthelp "PRIVMSG $chan :Mp3 maximal size limit is \002$mptigasize\002"
...
proc searchlagu {nick uhost hand chan text} {
	...
	if {![channel get $chan ytconvert] } { return 0 }
	if {$text == ""} { puthelp "PRIVMSG $chan :Syntax:\002 .cari <result number> <song> \002" ; return 0 }
	if {![regexp {[0-9]} [lindex $text 0]]} { puthelp "PRIVMSG $chan :Syntax:\002 .cari <result number> <song> \002" ; return 0 }
	...
	set ids [getinfocari $url]
	puthelp "PRIVMSG $chan :Sending \002$linktotal links\002 to private message."
...
proc limitdownloadmp3txt {nick host hand chan text} {
	global path yturl mptigasize mptigaruncmd
	if {$mptigasize != ""} {
		if {![matchattr $nick n]} {
...
proc limitdownloadmp3link {nick host hand chan text} {
	global path mptigasize mptigaruncmdd
	if {$mptigasize != ""} {
		if {![matchattr $nick n]} {
			catch {exec youtube-dl --max-filesize $mptigasize --prefer-ffmpeg --restrict-filenames --ignore-config --no-cache-dir --no-resize-buffer --no-playlist --no-mtime --no-color --no-mark-watched --no-call-home --no-warnings --youtube-skip-dash-manifest -w -x --audio-format mp3 --audio-quality 0 -o "$path/public_html/%(title)s.%(ext)s" $text} mptigaruncmdd
		} else {
			catch {exec youtube-dl --prefer-ffmpeg --restrict-filenames --ignore-config --no-cache-dir --no-resize-buffer --no-playlist --no-mtime --no-color --no-mark-watched --no-call-home --no-warnings --youtube-skip-dash-manifest -w -x --audio-format mp3 --audio-quality 0 -o "$path/public_html/%(title)s.%(ext)s" $text} mptigaruncmdd
		}
	} else {
		catch {exec youtube-dl --prefer-ffmpeg --restrict-filenames --ignore-config --no-cache-dir --no-resize-buffer --no-playlist --no-mtime --no-color --no-mark-watched --no-call-home --no-warnings --youtube-skip-dash-manifest -w -x --audio-format mp3 --audio-quality 0 -o "$path/public_html/%(title)s.%(ext)s" $text} mptigaruncmdd
	}
}
proc limitdownloadmp4txt {nick host hand chan text} {
	global path yturl mpempatsize mpempatruncmd
	if {$mpempatsize != ""} {
		if {![matchattr $nick n]} {
...
proc limitdownloadmp4link {nick host hand chan text} {
	global path mpempatsize mpempatruncmdd
	if {$mpempatsize != ""} {
		if {![matchattr $nick n]} {
